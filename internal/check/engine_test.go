...
func TestEngine(t *testing.T) {
	...
	t.Run("respects max depth", func(t *testing.T) {
		...
		assert.Equal(t, reg.Config(ctx).ReadAPIMaxDepth(), 5)
		...
		require.NoError(t, reg.Config(ctx).Set(config.KeyReadMaxDepth, 2))
		res, err = e.SubjectIsAllowed(ctx, userHasAccess, 3)
		require.NoError(t, err)
		assert.False(t, res)

		// global max-depth takes precedence and max-depth=3 is enough
		require.NoError(t, reg.Config(ctx).Set(config.KeyReadMaxDepth, 3))
